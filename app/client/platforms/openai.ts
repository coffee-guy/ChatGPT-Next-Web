...
export class ChatGPTApi {
    ...
    async createAudioTranscriptions(audioFile: Blob) {
        let finalPath = this.path(OpenaiPath.TranscriptionPath);
        console.log("createAudioTranscriptions:", finalPath);
        // 创建一个 FormData 实例
        const formData = new FormData();
        // 添加文件
        ...
        if (res.ok) {
            // 请求成功，返回解析后的JSON
            const result = camelizeKeys(await res.json()) as { "text": string };
            console.log("[OPENAI] createAudioTranscriptions|OK|:", result);
            return result;
        } else {
            // 请求失败，抛出错误或返回错误信息
            const errorInfo = await res.json();
            console.error("[OPENAI] createAudioTranscriptions|FAIL|", res);
            throw new Error(`Error createAudioTranscriptions: ${errorInfo.message}`);
        }
    }
